"""
GameController — the bridge between scenes and DB/logic.
Scenes call methods here; this class talks to the DB and updates GameState.
"""
...
class GameController:
    ...
    def start_run(self) -> None:
        """Create fresh player row in DB, reset state."""
        player = self.db.StartRun(self.state.user_id)
    ...
    def refresh_team(self) -> None:
        rows = self.db.GetTeam(self.state.user_id)
        self.state.load_team(rows)

    def refresh_shop(self) -> None:
        rows = self.db.GetShopOffer(self.state.turn)
    ...
    def buy_card(self, shop_card, slot: int) -> bool:
        ...
        player = self.db.GetPlayer(self.state.user_id)
        ...
        self.db.BuyCard(self.state.user_id, shop_card.card_id, slot)
        self.db.UpdatePlayer(self.state.user_id, {"gold": player["gold"] - cost})
        self.refresh_team()
    ...
    def sell_card(self, slot: int) -> bool:
        ...
        if card is None:
            return False
        self.db.SellCard(card.id, self.state.user_id)
        self.refresh_team()
        player = self.db.GetPlayer(self.state.user_id)
    ...
    def move_card(self, from_slot: int, to_slot: int) -> bool:
        ...
        if card is None or self.state.team[to_slot] is not None:
            return False
        self.db.MoveCard(card.id, to_slot)
        self.refresh_team()
        return True

    def reroll_shop(self) -> bool:
        if self.db.RerollShop(self.state.user_id):
            player = self.db.GetPlayer(self.state.user_id)
            self.state.gold = player["gold"]
            self.refresh_shop()
            return True
    ...
    def find_match(self) -> dict:
        """Calls atomic RPC. Returns game_manager row."""
        match = self.db.FindOrCreateMatch(self.state.user_id, self.state.turn)
        self.state.match = match
        return match

    def poll_match(self) -> dict | None:
        """Poll until opponent joins. Returns updated match or None."""
        if not self.state.match:
            return None
        match = self.db.GetMatch(self.state.match["id"])
        self.state.match = match
        return match

    def signal_shop_ready(self) -> None:
        """Player clicked Ready in shop — increment counter on match row."""
        if not self.state.match:
            return
        match = self.db.GetMatch(self.state.match["id"])
    ...
    def poll_both_ready(self) -> bool:
        """Returns True when both players have clicked ready."""
    ...
    def run_battle(self) -> dict:
        ...
        self.db.ResolveMatch(match["id"], winner_id)

        # Update player health, advance turn
        player   = self.db.GetPlayer(my_id)
