...
class RegisterScene(BaseScene):
    def __init__(self, game):
        ...
        self.username_input = TextInput(
            (MIDDLE_WIDTH - 200, 380, 400, 50), "Choose username", font=self.font)
        self.password_input = TextInput(
            (MIDDLE_WIDTH - 200, 460, 400, 50), "Choose password",
            password=True, font=self.font)
    ...
    def do_register(self):
        ...
        user   = self.game.db.register(username, hashed)   # lowercase — fixed
        if not user:
